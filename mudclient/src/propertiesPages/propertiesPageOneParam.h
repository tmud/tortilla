...
public:
     ...
     PropertyOneParam() : propValues(NULL), propGroups(NULL), m_filterMode(0), m_deleted(false), m_update_mode(false), dlg_state(NULL) {}
...
private:
    ...
    LRESULT OnAddElement(WORD, WORD, HWND, BOOL&)
    {
        ...
        if (index == -1)
        {
            ...
            m_list_values.insert(index, pattern, L"", m_currentGroup);
            m_list.addItem(index, 0, pattern);
            m_list.addItem(index, 1, m_currentGroup);
        }
        else
        {
    ...
    LRESULT OnUpElement(WORD, WORD, HWND, BOOL&)
    {
        ...
        ud.up(m_list, m_list_values, true);
        return 0;
    }
    ...
    {
        propertiesUpDown<tstring> ud;
        ud.down(m_list, m_list_values, true);
        return 0;
    }
