...
-- если id не указан (только для раздевания), то идет поиск экипировки для снятия по слотам
undress={
{ key="Вы прекратили использовать %1." },
}

-- Строка с которой начинается список инвентаря (заканчивается пустой строкой)
inventory_begin = "^Вы несете:"

-- триггеры для добавления в инвентарь (кроме случаев одевания/снятия экипировки)
inventory_in = {
